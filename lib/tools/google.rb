module Tools
  class Google
    ...
    def self.update_books_by_genre
      books_for_all_genres.each do |item|
        isbn = isbn_10_from_api(item['volumeInfo']['industryIdentifiers'])
        next if isbn.nil? || Book.exists?(isbn)
        create_book(genre.id)
      end
    end

    def self.assign_images_to_all_books
    ...
    def create_book(genre_id = 20)
      @book = Book.new
      return unless prep_book_for_update(@book)
