    ...
    @books = []
      ...
      goog_response = HTTParty.get("https://www.googleapis.com/books/v1/volumes?q=#{params[:search]}") #&key=#{ENV['GBOOKS_KEY']}
        ...
        goog_response.parsed_response["items"][0..5].each do |result|
          ...
          @books << { title: result["volumeInfo"]["title"], author: result["volumeInfo"]["authors"], description: result["volumeInfo"]["description"], isbn: result["volumeInfo"]["industryIdentifiers"][0]["identifier"], book_image: book_img }
        end
